    ...
    <div className={wamometer.thermo}>
      <div className={wamometer.thermoMarkers}>
        {leftMarkers.map((marker, index) => (
          <div
            key={index}
            className={`${wamometer.thermoMarker} ${
              index >= markerSteps - value / (100 / markerSteps)
                ? wamometer.thermoMarkerActive
                : ""
            }`}
          >
            {leftMarkers[index] && (
              <p aria-hidden className={wamometer.leftMarker}>
                {leftMarkers[index]}
              </p>
            )}
            {rightMarkers[index] && (
              <p aria-hidden className={wamometer.rightMarker}>
                {rightMarkers[index]}
              </p>
            )}
          </div>
      ...
      {value > 0 && (
        <div>
          ...
          <div className={wamometer.thermoTrackContainer}>
            <div
              className={wamometer.thermoTrackFront}
              style={{ height: `${Math.min(value + 1, 100)}%` }}
            ></div>
          </div>
        </div>
      )}
